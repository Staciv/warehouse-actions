...
const sha256Fallback = (input: string) => {
  ...
  dataView.setUint32(paddedLength - 8, Math.floor(bitLength / 2 ** 32), false);
...
export const sha256 = async (input: string) => {
  ...
  if (subtle) {
    const digest = await subtle.digest('SHA-256', encode(input));
